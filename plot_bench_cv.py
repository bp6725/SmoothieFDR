...
def plot_combined_loss_convergence(results_dict, save_path=None):
    ...
    for (ds_name, res), color in zip(results_dict.items(), colors):
        ...
        plt.plot(iterations, loss_norm, '-', linewidth=2.5, color=color, label=ds_name)

    plt.xlabel('Iteration')
    plt.ylabel('Normalized Loss')
    plt.title('Global Loss Convergence Across All Datasets', fontsize=20, fontweight='bold')
